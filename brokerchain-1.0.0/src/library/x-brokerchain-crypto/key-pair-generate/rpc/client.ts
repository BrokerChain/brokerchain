...
export async function rpc_key_pair_generate<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_key_pair_generate");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/x-brokerchain-crypto/key-pair-generate`,
                data: input
            },
            {
