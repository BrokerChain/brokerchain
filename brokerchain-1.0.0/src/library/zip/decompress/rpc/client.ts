...
export async function rpc_decompress<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_decompress");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/zip/decompress`,
                data: input
            },
            {
