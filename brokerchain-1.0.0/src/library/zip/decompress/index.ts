...
import { decompress as target_fun } from "./export.js";
import { rpc_decompress as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(plog: Logger, cb: { ok: (v: { from_file: string; to_path: string }) => R; fail: (err: Error) => R }): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { from_file: string; to_path: string } = {
            from_file: await input_from_file(log.sub("from_file"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            to_path: await input_to_path(log.sub("to_path"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_from_file<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_from_file");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("from_file", { allow_empty: true });
        return cb.ok(v);
    }

    async function input_to_path<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_to_path");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("to_path", { allow_empty: true });
        return cb.ok(v);
    }
}
