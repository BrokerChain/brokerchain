...
export function check_input<R>(plog: Logger, v: any, cb: { ok: () => R; fail: (err: Error) => R }): R {
    ...
    try {
        ...
        Object.keys(v).forEach((field) => {
            if (field === "from_file") return;
            if (field === "to_path") return;
            throw new Error("v contains unknown field: " + field);
        });

        log.println("v.from_file must be string");
        if (typeof v.from_file !== "string") {
            throw new Error("v.from_file is not string");
        }

        log.println("v.to_path must be string");
        if (typeof v.to_path !== "string") {
            throw new Error("v.to_path is not string");
        }
    } catch (err) {
        log.error(err);
        return cb.fail(err);
