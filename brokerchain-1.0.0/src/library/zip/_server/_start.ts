...
import { handle_rpc_compress } from "../compress/rpc/handle.js";
import { handle_rpc_decompress } from "../decompress/rpc/handle.js";
...
export function _start(plog: Logger, opt: { host: string; http_port: number; https_port: number; setup?: (exp_app: express.Express) => void }) {
    ...
    exp_app.post("/library/zip/compress", (req, res) => {
        const input = req.body;
        const req_log = log.sub("post.library.zip.compress");
        handle_rpc_compress(req_log, input, {
            invalid_input: (err) => {
                // bad request
                req_log.error(err);
                res.status(400);
    ...
    exp_app.post("/library/zip/decompress", (req, res) => {
        const input = req.body;
        const req_log = log.sub("post.library.zip.decompress");
        handle_rpc_decompress(req_log, input, {
            invalid_input: (err) => {
                // bad request
                req_log.error(err);
                res.status(400);
