...
import { disk_collection_item_del as target_fun } from "./export.js";
import { rpc_disk_collection_item_del as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { namespace: string; key: string; item_id: string }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { namespace: string; key: string; item_id: string } = {
            ...
            item_id: await input_item_id(log.sub("item_id"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_item_id<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_item_id");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("item_id", { allow_empty: true });
        return cb.ok(v);
    }
}
