...
import { disk_collection_item_get as target_fun } from "./export.js";
import { rpc_disk_collection_item_get as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { namespace: string; key: string; id: string }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { namespace: string; key: string; id: string } = {
