...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { file: string; args: string[]; text?: string; cwd?: string }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { file: string; args: string[]; text?: string; cwd?: string } = {
            file: await input_file(log.sub("file"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            args: await input_args(log.sub("args"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            text: await skip_or_input("text", () =>
                input_text(log.sub("text"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
            ...
            cwd: await skip_or_input("cwd", () =>
                input_cwd(log.sub("cwd"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
    ...
    async function input_args<R>(plog: Logger, cb: { ok: (v: string[]) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_args");
        const length = await prompts.input_number("(array.length) args");
        const list: string[] = [];
    ...
    async function input_cwd<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_cwd");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("cwd", { allow_empty: true });
        return cb.ok(v);
    }
