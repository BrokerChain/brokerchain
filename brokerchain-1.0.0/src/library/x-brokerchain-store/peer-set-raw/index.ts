...
import { peer_set_raw as target_fun } from "./export.js";
import { rpc_peer_set_raw as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: {
        ok: (v: { peer: { id: string; fake?: boolean; create_time: string; update_time: string; address: string; port: number } }) => R;
        fail: (err: Error) => R;
    }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { peer: { id: string; fake?: boolean; create_time: string; update_time: string; address: string; port: number } } = {
            peer: await input_peer(log.sub("peer"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
