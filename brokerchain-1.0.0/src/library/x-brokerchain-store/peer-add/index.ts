...
import { peer_add as target_fun } from "./export.js";
import { rpc_peer_add as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { peer: { fake?: boolean; address: string; port: number } }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { peer: { fake?: boolean; address: string; port: number } } = {
    ...
    async function input_peer<R>(plog: Logger, cb: { ok: (v: { fake?: boolean; address: string; port: number }) => R; fail: (err: Error) => R }): Promise<R> {
