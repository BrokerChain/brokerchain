...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { node: { id: string; fake?: boolean; create_time: string; update_time: string } }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { node: { id: string; fake?: boolean; create_time: string; update_time: string } } = {
    ...
    async function input_node<R>(
        plog: Logger,
        cb: { ok: (v: { id: string; fake?: boolean; create_time: string; update_time: string }) => R; fail: (err: Error) => R }
    ): Promise<R> {
        const log = plog.sub("input_node");
        try {
            var v: { id: string; fake?: boolean; create_time: string; update_time: string } = {
