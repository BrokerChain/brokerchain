...
export async function rpc_peer_set<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_peer_set");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/x-brokerchain-store/peer-set`,
                data: input
            },
            {
