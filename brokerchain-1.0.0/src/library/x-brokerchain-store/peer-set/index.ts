...
import { peer_set as target_fun } from "./export.js";
import { rpc_peer_set as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { peer: { id: string; fake?: boolean; address: string; port: number } }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { peer: { id: string; fake?: boolean; address: string; port: number } } = {
    ...
    async function input_peer<R>(
        plog: Logger,
        cb: { ok: (v: { id: string; fake?: boolean; address: string; port: number }) => R; fail: (err: Error) => R }
    ): Promise<R> {
        const log = plog.sub("input_peer");
        try {
            var v: { id: string; fake?: boolean; address: string; port: number } = {
