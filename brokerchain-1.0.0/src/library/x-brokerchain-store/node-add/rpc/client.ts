...
export async function rpc_node_add<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_node_add");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/x-brokerchain-store/node-add`,
                data: input
            },
            {
