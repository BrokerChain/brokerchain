...
import { Input, Callback, OutputNone, OutputOk } from "../type.js";
import { RpcResult } from "./type.js";

export async function rpc_peer_get_raw<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_peer_get_raw");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/x-brokerchain-store/peer-get-raw`,
                data: input
            },
            {
                ok: (rpc_result: RpcResult) => {
                    ...
                    if (rpc_result.none) {
                        log.println("none");
                        const output: OutputNone = rpc_result.none;
