...
export function _copy_output_none(v: any): OutputNone {
    if (typeof v === "object" && v !== null) {
        const obj = {
            peer: copy_peer(v.peer)
        };
        return obj;
    } else {
        return { peer: { id: "", create_time: "", update_time: "", address: "", port: 0 } };
    }

    function copy_peer(v: any): { id: string; fake?: boolean; create_time: string; update_time: string; address: string; port: number } {
        if (typeof v === "object" && v !== null) {
            const obj = {
                id: copy_id(v.id),
