...
import { make_page_node } from "../make-page-node/export.js";
...
export async function core<R>(log: Logger, input: Input, cb: Callback<R>): Promise<R> {
    const { html } = await make_page_node(
        log,
        {},
        {
            ok: (output) => {
    ...
    await write_text_file(log, path.resolve(dir.root, "node/index.html"), html, {
        ok: () => {
            // ignore
        },
        fail: (err) => {
    ...
    await vite.build(
        log,
        {
            ...
            entry_html_list: [
                {
                    name: "node",
                    path: path.resolve(dir.root, "node/index.html")
                }
            ],
    ...
    async function clean() {
        await del_dir(log, path.resolve(dir.root, "node"), {
            ok: () => {
                // ignore
            },
            fail: (err) => {
