...
import { rpc_run } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { build: boolean; host?: string; http_port?: number; https_port?: number }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { build: boolean; host?: string; http_port?: number; https_port?: number } = {
            build: await input_build(log.sub("build"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            host: await skip_or_input("host", () =>
                input_host(log.sub("host"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
            ...
            http_port: await skip_or_input("http_port", () =>
                input_http_port(log.sub("http_port"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
            ...
            https_port: await skip_or_input("https_port", () =>
                input_https_port(log.sub("https_port"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
    ...
    async function input_build<R>(plog: Logger, cb: { ok: (v: boolean) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_build");
        const v = await prompts.input_boolean("build");
        return cb.ok(v);
    }

    async function input_host<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_host");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("host", { allow_empty: true });
        return cb.ok(v);
    }

    async function input_http_port<R>(plog: Logger, cb: { ok: (v: number) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_http_port");
        // FIXME implement all number constrains here
        const v = await prompts.input_number("http_port");
        return cb.ok(v);
    }

    async function input_https_port<R>(plog: Logger, cb: { ok: (v: number) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_https_port");
        // FIXME implement all number constrains here
        const v = await prompts.input_number("https_port");
        return cb.ok(v);
    }
