...
import { run as target_fun } from "./export.js";
import { rpc_run as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    ...
    try {
        var v: { build: boolean; host: string; http_port: number; https_port: number } = {
            ...
            host: await input_host(log.sub("host"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            http_port: await input_http_port(log.sub("http_port"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            https_port: await input_https_port(log.sub("https_port"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
