...
import { make_page_input_phone } from "../make-page-input-phone/export.js";
...
export async function core<R>(log: Logger, input: Input, cb: Callback<R>): Promise<R> {
    const { html } = await make_page_input_phone(
        log,
        {},
        {
            ok: (output) => {
    ...
    await write_text_file(log, path.resolve(dir.root, "input-phone/index.html"), html, {
        ok: () => {
            // ignore
        },
        fail: (err) => {
    ...
    await vite.build(
        log,
        {
            ...
            entry_html_list: [
                {
                    name: "input-phone",
                    path: path.resolve(dir.root, "input-phone/index.html")
                }
            ],
    ...
    async function clean() {
        await del_dir(log, path.resolve(dir.root, "input-phone"), {
            ok: () => {
                // ignore
            },
            fail: (err) => {
