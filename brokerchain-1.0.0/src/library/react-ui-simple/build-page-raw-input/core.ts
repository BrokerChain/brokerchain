...
import { make_page_raw_input } from "../make-page-raw-input/export.js";
...
export async function core<R>(log: Logger, input: Input, cb: Callback<R>): Promise<R> {
    const { html } = await make_page_raw_input(
        log,
        {},
        {
            ok: (output) => {
    ...
    await write_text_file(log, path.resolve(dir.root, "raw-input/index.html"), html, {
        ok: () => {
            // ignore
        },
        fail: (err) => {
    ...
    await vite.build(
        log,
        {
            ...
            entry_html_list: [
                {
                    name: "raw-input",
                    path: path.resolve(dir.root, "raw-input/index.html")
                }
            ],
    ...
    async function clean() {
        await del_dir(log, path.resolve(dir.root, "raw-input"), {
            ok: () => {
                // ignore
            },
            fail: (err) => {
