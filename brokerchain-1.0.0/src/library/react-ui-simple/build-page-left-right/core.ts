...
export async function core<R>(log: Logger, input: Input, cb: Callback<R>): Promise<R> {
    ...
    await write_text_file(log, path.resolve(dir.root, "left-right/index.html"), html, {
        ok: () => {
            // ignore
        },
        fail: (err) => {
    ...
    await vite.build(
        log,
        {
            ...
            entry_html_list: [
                {
                    name: "left-right",
                    path: path.resolve(dir.root, "left-right/index.html")
                }
            ],
    ...
    async function clean() {
        await del_dir(log, path.resolve(dir.root, "left-right"), {
            ok: () => {
                // ignore
            },
            fail: (err) => {
