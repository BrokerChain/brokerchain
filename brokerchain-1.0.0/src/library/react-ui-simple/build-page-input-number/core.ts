...
import { make_page_input_number } from "../make-page-input-number/export.js";
...
export async function core<R>(log: Logger, input: Input, cb: Callback<R>): Promise<R> {
    const { html } = await make_page_input_number(
        log,
        {},
        {
            ok: (output) => {
    ...
    await write_text_file(log, path.resolve(dir.root, "input-number/index.html"), html, {
        ok: () => {
            // ignore
        },
        fail: (err) => {
    ...
    await vite.build(
        log,
        {
            ...
            entry_html_list: [
                {
                    name: "input-number",
                    path: path.resolve(dir.root, "input-number/index.html")
                }
            ],
    ...
    async function clean() {
        await del_dir(log, path.resolve(dir.root, "input-number"), {
            ok: () => {
                // ignore
            },
            fail: (err) => {
