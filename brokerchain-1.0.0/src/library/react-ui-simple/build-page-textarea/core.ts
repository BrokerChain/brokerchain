...
import { make_page_textarea } from "../make-page-textarea/export.js";
...
export async function core<R>(log: Logger, input: Input, cb: Callback<R>): Promise<R> {
    const { html } = await make_page_textarea(
        log,
        {},
        {
            ok: (output) => {
    ...
    await write_text_file(log, path.resolve(dir.root, "textarea/index.html"), html, {
        ok: () => {
            // ignore
        },
        fail: (err) => {
    ...
    await vite.build(
        log,
        {
            ...
            entry_html_list: [
                {
                    name: "textarea",
                    path: path.resolve(dir.root, "textarea/index.html")
                }
            ],
    ...
    async function clean() {
        await del_dir(log, path.resolve(dir.root, "textarea"), {
            ok: () => {
                // ignore
            },
            fail: (err) => {
