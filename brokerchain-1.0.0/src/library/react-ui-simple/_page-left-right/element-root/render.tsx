...
export function render(self: Root): React.ReactNode {
    ...
    const { vio, className, left, leftClassName, right, rightClassName, ...props } = self.props;

    return (
        <div className={clsx("grid grid-cols-2 gap-4", className)} {...props}>
            <div className={clsx("", leftClassName)}>{left}</div>
            <div className={clsx("", rightClassName)}>{right}</div>
        </div>
    );
}
