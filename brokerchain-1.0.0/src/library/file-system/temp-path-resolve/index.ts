...
import { temp_path_resolve as target_fun } from "./export.js";
import { rpc_temp_path_resolve as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(plog: Logger, cb: { ok: (v: { part_list: string[] }) => R; fail: (err: Error) => R }): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { part_list: string[] } = {
            part_list: await input_part_list(log.sub("part_list"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_part_list<R>(plog: Logger, cb: { ok: (v: string[]) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_part_list");
        const length = await prompts.input_number("(array.length) part_list");
        const list: string[] = [];
