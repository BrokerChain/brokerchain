...
import { unwatch as target_fun } from "./export.js";
import { rpc_unwatch as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(plog: Logger, cb: { ok: (v: { watcher_id: string }) => R; fail: (err: Error) => R }): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { watcher_id: string } = {
            watcher_id: await input_watcher_id(log.sub("watcher_id"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_watcher_id<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_watcher_id");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("watcher_id", { allow_empty: true });
        return cb.ok(v);
    }
}
