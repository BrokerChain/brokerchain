...
import { chmod as target_fun } from "./export.js";
import { rpc_chmod as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(plog: Logger, cb: { ok: (v: { path: string; mode: number }) => R; fail: (err: Error) => R }): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { path: string; mode: number } = {
    ...
    async function input_mode<R>(plog: Logger, cb: { ok: (v: number) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_mode");
        // FIXME implement all number constrains here
        const v = await prompts.input_number("mode");
        return cb.ok(v);
    }
}
