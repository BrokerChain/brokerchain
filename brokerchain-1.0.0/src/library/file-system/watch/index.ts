...
import { watch as target_fun } from "./export.js";
import { rpc_watch as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    ...
    try {
        var v: {
            ...
            persistent: await skip_or_input("persistent", () =>
                input_persistent(log.sub("persistent"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
            ...
            recursive: await skip_or_input("recursive", () =>
                input_recursive(log.sub("recursive"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
            ...
            encoding: await skip_or_input("encoding", () =>
                input_encoding(log.sub("encoding"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
    ...
    async function input_persistent<R>(plog: Logger, cb: { ok: (v: boolean) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_persistent");
        const v = await prompts.input_boolean("persistent");
        return cb.ok(v);
    }

    async function input_recursive<R>(plog: Logger, cb: { ok: (v: boolean) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_recursive");
        const v = await prompts.input_boolean("recursive");
        return cb.ok(v);
    }

    async function input_encoding<R>(
        plog: Logger,
        cb: {
            ok: (
                v: "ascii" | "utf8" | "utf-8" | "utf16le" | "utf-16le" | "ucs2" | "ucs-2" | "base64" | "base64url" | "latin1" | "binary" | "hex" | "buffer"
            ) => R;
            fail: (err: Error) => R;
        }
    ): Promise<R> {
        const log = plog.sub("input_encoding");
        // FIXME implement all string constrains here
        const v: "ascii" | "utf8" | "utf-8" | "utf16le" | "utf-16le" | "ucs2" | "ucs-2" | "base64" | "base64url" | "latin1" | "binary" | "hex" | "buffer" =
            await prompts.input_string_enum("encoding", [
                "ascii",
                "utf8",
                "utf-8",
                "utf16le",
