...
import { file_ls as target_fun } from "./export.js";
import { rpc_file_ls as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(plog: Logger, cb: { ok: (v: { dir: string; recursive: boolean }) => R; fail: (err: Error) => R }): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { dir: string; recursive: boolean } = {
            dir: await input_dir(log.sub("dir"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            recursive: await input_recursive(log.sub("recursive"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_dir<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_dir");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("dir", { allow_empty: true });
        return cb.ok(v);
    }
