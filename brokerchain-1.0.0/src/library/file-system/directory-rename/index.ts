...
import { directory_rename as target_fun } from "./export.js";
import { rpc_directory_rename as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { base_path: string; name_from: string; name_to: string }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { base_path: string; name_from: string; name_to: string } = {
            base_path: await input_base_path(log.sub("base_path"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            name_from: await input_name_from(log.sub("name_from"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            name_to: await input_name_to(log.sub("name_to"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_base_path<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_base_path");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("base_path", { allow_empty: true });
        return cb.ok(v);
    }

    async function input_name_from<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_name_from");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("name_from", { allow_empty: true });
        return cb.ok(v);
    }

    async function input_name_to<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_name_to");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("name_to", { allow_empty: true });
        return cb.ok(v);
    }
}
