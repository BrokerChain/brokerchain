...
import { directory_copy as target_fun } from "./export.js";
import { rpc_directory_copy as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(plog: Logger, cb: { ok: (v: { from: string; to: string }) => R; fail: (err: Error) => R }): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { from: string; to: string } = {
            from: await input_from(log.sub("from"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            to: await input_to(log.sub("to"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_from<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_from");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("from", { allow_empty: true });
        return cb.ok(v);
    }

    async function input_to<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_to");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("to", { allow_empty: true });
        return cb.ok(v);
    }
}
