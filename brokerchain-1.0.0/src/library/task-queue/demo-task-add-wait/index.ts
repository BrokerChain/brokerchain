...
import { demo_task_add_wait as target_fun } from "./export.js";
import { rpc_demo_task_add_wait as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { queue_id: string; mode: "resolve" | "15s-resolve" | "reject" | "15s-reject" | "infinite" }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { queue_id: string; mode: "resolve" | "15s-resolve" | "reject" | "15s-reject" | "infinite" } = {
            queue_id: await input_queue_id(log.sub("queue_id"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            mode: await input_mode(log.sub("mode"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_queue_id<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_queue_id");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("queue_id", { allow_empty: true });
        return cb.ok(v);
    }

    async function input_mode<R>(
        plog: Logger,
        cb: { ok: (v: "resolve" | "15s-resolve" | "reject" | "15s-reject" | "infinite") => R; fail: (err: Error) => R }
    ): Promise<R> {
        const log = plog.sub("input_mode");
        // FIXME implement all string constrains here
        const v: "resolve" | "15s-resolve" | "reject" | "15s-reject" | "infinite" = await prompts.input_string_enum("mode", [
            "resolve",
            "15s-resolve",
            "reject",
            "15s-reject",
