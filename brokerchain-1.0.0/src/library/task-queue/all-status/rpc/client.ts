...
export async function rpc_all_status<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_all_status");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/task-queue/all-status`,
                data: input
            },
            {
