...
export async function rpc_task_add_wait<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_task_add_wait");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/task-queue/task-add-wait`,
                data: input
            },
            {
