...
import { task_wait as target_fun } from "./export.js";
import { rpc_task_wait as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(plog: Logger, cb: { ok: (v: { queue_id: string; task_id: string }) => R; fail: (err: Error) => R }): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { queue_id: string; task_id: string } = {
            ...
            task_id: await input_task_id(log.sub("task_id"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_task_id<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_task_id");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("task_id", { allow_empty: true });
        return cb.ok(v);
    }
}
