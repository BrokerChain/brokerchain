...
export async function rpc_queue_status<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_queue_status");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/task-queue/queue-status`,
                data: input
            },
            {
