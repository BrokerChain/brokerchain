...
export function check_input<R>(plog: Logger, v: any, cb: { ok: () => R; fail: (err: Error) => R }): R {
    ...
    try {
        ...
        Object.keys(v).forEach((field) => {
            ...
            if (field === "ciphered_content_hex") return;
        ...
        log.println("v.ciphered_content_hex must be string");
        if (typeof v.ciphered_content_hex !== "string") {
            throw new Error("v.ciphered_content_hex is not string");
        }
    } catch (err) {
        log.error(err);
        return cb.fail(err);
...
export function _copy_output_ok(v: any): OutputOk {
    if (typeof v === "object" && v !== null) {
        const obj = {
            plain_content_hex: copy_plain_content_hex(v.plain_content_hex)
        };
        return obj;
    } else {
    ...
    function copy_plain_content_hex(v: any): string {
