...
import { ecdh_x25519_secret_compute as target_fun } from "./export.js";
import { rpc_ecdh_x25519_secret_compute as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { my_private_key_hex: string; target_public_key_hex: string }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { my_private_key_hex: string; target_public_key_hex: string } = {
            my_private_key_hex: await input_my_private_key_hex(log.sub("my_private_key_hex"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            target_public_key_hex: await input_target_public_key_hex(log.sub("target_public_key_hex"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_my_private_key_hex<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_my_private_key_hex");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("my_private_key_hex", { allow_empty: true });
        return cb.ok(v);
    }

    async function input_target_public_key_hex<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_target_public_key_hex");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("target_public_key_hex", { allow_empty: true });
        return cb.ok(v);
    }
}
