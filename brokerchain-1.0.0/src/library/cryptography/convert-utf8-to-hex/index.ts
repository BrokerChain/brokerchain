...
import { convert_utf8_to_hex as target_fun } from "./export.js";
import { rpc_convert_utf8_to_hex as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(plog: Logger, cb: { ok: (v: { utf8: string }) => R; fail: (err: Error) => R }): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { utf8: string } = {
            utf8: await input_utf8(log.sub("utf8"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_utf8<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_utf8");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("utf8", { allow_empty: true });
        return cb.ok(v);
    }
}
