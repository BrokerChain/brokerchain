...
import { cipher_text as target_fun } from "./export.js";
import { rpc_cipher_text as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    ...
    try {
        var v: {
            ...
            plain_text: await input_plain_text(log.sub("plain_text"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_plain_text<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_plain_text");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("plain_text", { allow_empty: true });
        return cb.ok(v);
    }
}
