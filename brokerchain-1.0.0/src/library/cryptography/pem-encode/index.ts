...
import { pem_encode as target_fun } from "./export.js";
import { rpc_pem_encode as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(plog: Logger, cb: { ok: (v: { type: string; content: string }) => R; fail: (err: Error) => R }): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { type: string; content: string } = {
            type: await input_type(log.sub("type"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
            }),
            content: await input_content(log.sub("content"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_type<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_type");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("type", { allow_empty: true });
        return cb.ok(v);
    }

    async function input_content<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_content");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("content", { allow_empty: true });
        return cb.ok(v);
    }
}
