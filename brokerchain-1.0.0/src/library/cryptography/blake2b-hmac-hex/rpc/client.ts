...
export async function rpc_blake2b_hmac_hex<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_blake2b_hmac_hex");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/cryptography/blake2b-hmac-hex`,
                data: input
            },
            {
