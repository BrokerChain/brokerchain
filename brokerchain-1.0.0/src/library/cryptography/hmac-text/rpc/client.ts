...
export async function rpc_hmac_text<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_hmac_text");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/cryptography/hmac-text`,
                data: input
            },
            {
