...
export async function rpc_blake2s_hmac_hex<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_blake2s_hmac_hex");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/cryptography/blake2s-hmac-hex`,
                data: input
            },
            {
