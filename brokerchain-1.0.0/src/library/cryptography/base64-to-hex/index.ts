...
import { base64_to_hex as target_fun } from "./export.js";
import { rpc_base64_to_hex as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(plog: Logger, cb: { ok: (v: { base64: string }) => R; fail: (err: Error) => R }): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { base64: string } = {
            base64: await input_base64(log.sub("base64"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_base64<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_base64");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("base64", { allow_empty: true });
        return cb.ok(v);
    }
}
