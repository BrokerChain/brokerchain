...
import { cipher_hex as target_fun } from "./export.js";
import { rpc_cipher_hex as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    ...
    try {
        var v: {
            ...
            plain_content_hex: await input_plain_content_hex(log.sub("plain_content_hex"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_plain_content_hex<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_plain_content_hex");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("plain_content_hex", { allow_empty: true });
        return cb.ok(v);
    }
}
