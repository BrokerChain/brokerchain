...
import { random_hex as target_fun } from "./export.js";
import { rpc_random_hex as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(plog: Logger, cb: { ok: (v: { bytes: number }) => R; fail: (err: Error) => R }): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { bytes: number } = {
            bytes: await input_bytes(log.sub("bytes"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_bytes<R>(plog: Logger, cb: { ok: (v: number) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_bytes");
        // FIXME implement all number constrains here
        const v = await prompts.input_number("bytes");
        return cb.ok(v);
    }
}
