...
import { rsa_key_pair_generate as target_fun } from "./export.js";
import { rpc_rsa_key_pair_generate as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(plog: Logger, cb: { ok: (v: { bits?: number }) => R; fail: (err: Error) => R }): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { bits?: number } = {
            bits: await skip_or_input("bits", () =>
                input_bits(log.sub("bits"), {
                    ok: (v) => v,
                    fail: (err) => {
                        throw err;
                    }
    ...
    async function input_bits<R>(plog: Logger, cb: { ok: (v: number) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_bits");
        // FIXME implement all number constrains here
        const v = await prompts.input_number("bits");
        return cb.ok(v);
    }
