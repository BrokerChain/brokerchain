...
export async function rpc_rsa_key_pair_generate<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_rsa_key_pair_generate");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/cryptography/rsa-key-pair-generate`,
                data: input
            },
            {
