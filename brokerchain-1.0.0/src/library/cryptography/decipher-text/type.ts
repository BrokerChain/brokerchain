...
export function check_input<R>(plog: Logger, v: any, cb: { ok: () => R; fail: (err: Error) => R }): R {
    ...
    try {
        ...
        Object.keys(v).forEach((field) => {
            ...
            if (field === "ciphered_text_hex") return;
        ...
        log.println("v.ciphered_text_hex must be string");
        if (typeof v.ciphered_text_hex !== "string") {
            throw new Error("v.ciphered_text_hex is not string");
        }
    } catch (err) {
        log.error(err);
        return cb.fail(err);
...
export function _copy_output_ok(v: any): OutputOk {
    if (typeof v === "object" && v !== null) {
        const obj = {
            plain_text: copy_plain_text(v.plain_text)
        };
        return obj;
    } else {
        return { plain_text: "" };
    ...
    function copy_plain_text(v: any): string {
