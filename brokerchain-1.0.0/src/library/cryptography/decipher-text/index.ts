...
import { decipher_text as target_fun } from "./export.js";
import { rpc_decipher_text as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    ...
    try {
        var v: {
            ...
            ciphered_text_hex: await input_ciphered_text_hex(log.sub("ciphered_text_hex"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_ciphered_text_hex<R>(plog: Logger, cb: { ok: (v: string) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_ciphered_text_hex");
        // FIXME implement all string constrains here
        const v = await prompts.input_string("ciphered_text_hex", { allow_empty: true });
        return cb.ok(v);
    }
}
