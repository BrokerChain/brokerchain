...
export async function rpc_decipher_text<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_decipher_text");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/cryptography/decipher-text`,
                data: input
            },
            {
