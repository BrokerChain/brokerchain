...
import { blake2b_hash_hex as target_fun } from "./export.js";
import { rpc_blake2b_hash_hex as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(plog: Logger, cb: { ok: (v: { content_hex: string }) => R; fail: (err: Error) => R }): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { content_hex: string } = {
