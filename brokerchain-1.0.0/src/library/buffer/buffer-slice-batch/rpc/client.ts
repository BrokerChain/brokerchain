...
import { Input, Callback, OutputEmpty, OutputOk } from "../type.js";
import { RpcResult } from "./type.js";

export async function rpc_buffer_slice_batch<R>(plog: Logger, opts: { server: string; input: Input }, cb: Callback<R>): Promise<R> {
    const log = plog.sub("rpc_buffer_slice_batch");
    ...
    try {
        ...
        return await post_json(
            log,
            {
                url: `${prefix}/library/buffer/buffer-slice-batch`,
                data: input
            },
            {
                ok: (rpc_result: RpcResult) => {
                    ...
                    if (rpc_result.empty) {
                        log.println("empty");
                        const output: OutputEmpty = rpc_result.empty;
