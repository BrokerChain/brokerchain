...
import { buffer_slice as target_fun } from "./export.js";
import { rpc_buffer_slice as rpc_target_fun } from "./rpc/client.js";
...
async function make_input_from_prompts<R>(
    plog: Logger,
    cb: { ok: (v: { hex: string; byte_start: number; byte_end: number }) => R; fail: (err: Error) => R }
): Promise<R> {
    const log = plog.sub("make_input_from_prompts");
    try {
        var v: { hex: string; byte_start: number; byte_end: number } = {
            ...
            byte_end: await input_byte_end(log.sub("byte_end"), {
                ok: (v) => v,
                fail: (err) => {
                    throw err;
                }
    ...
    async function input_byte_end<R>(plog: Logger, cb: { ok: (v: number) => R; fail: (err: Error) => R }): Promise<R> {
        const log = plog.sub("input_byte_end");
        // FIXME implement all number constrains here
        const v = await prompts.input_number("byte_end");
        return cb.ok(v);
    }
}
